...
def get_data():
    ...
    * Possible refactor that loading data can be used for training AND predicting based on certain parameters
    ...
    df_wijk_sub = rename_and_subset_cols(df=df_wijk,
                                         dict_rename=settings.get_data['DICT_WIJK_COLS_RENAMED'],
    ...
    df_wijk_total = df_wijk_total.set_index(settings.get_data['LIST_INDEX_WIJK'])

    print("Get 'Huishoudens' data")
    df_households = get_and_combine_cbs_tables(dict_tables=settings.get_data['DICT_TABLES_HUISHOUDEN'],
                                               url=settings.get_date['CBS_OPEN_URL'])
